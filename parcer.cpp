    ...
    double calcExpr(TTreeNode* tree)//calculate the result
    {
...
TTreeNode* parseUnaryExpr(TLexem* lexemsArr, int lexemsAm, int* posAdr)
    ...
    //create binary tree of TLexems from an array (relying on the precedence of math operations)
    ...
    TLexem* lexer(char* expr, int* lexemsAmAdr)//Split string to TLexem objects and check if it's correct and return array of TLexems
    {
        VarsAm = 0;
        int pos = 0;
        int arrSize = 100;
    ...
    int isFunc(char* w)//check syntax
    {
        string word(w);
    ...
    int isConst(char* w)//check syntax
    {
        string word(w);
        for (int i = 0; i < ConstsAm; i++)
        {

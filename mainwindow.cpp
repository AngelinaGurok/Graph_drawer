...
} //input math functions(sin,cos,tg,ctg,abs,sqrt)
...
void MainWindow::digits_numbers(){
    ...
    ui->functionShow->setText(new_label);
} //input numbers, math operations, constants, math variable
...
double MainWindow::f(double x, QString QExpr)//parse expression and calculate result
{
    correct();
    int tokensAm;
    QByteArray ch = QExpr.toLocal8Bit();//to string
    char *expr = ch.data();
    TLexem* tokens;
    tokens = lexer(expr, &tokensAm);//split to TLexems
    TTreeNode* tree;

    try{
        tree = parser(tokens, tokensAm);//create tree
    }
    ...
    inputVars(x);
    return calcExpr(tree);//calculate (get double number)
}
...
void MainWindow::getData()//get max and min of x and y and get step
{
...
void MainWindow::drawGraph(bool notEmpty)
{
    ...
    for(double i = leftX;i<=rightX;i+=1.0)//drawing x y axes and single segments
...
void MainWindow::on_save_clicked()//save the image of plot
{
